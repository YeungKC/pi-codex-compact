...
Compaction performed at the first request after a Codex model change, before that request is sent. Selecting a model alone does not compact. If both models expose the same compaction hash, the transition is skipped; when Pi does not expose hashes, the extension conservatively treats a model ID change as a transition.
